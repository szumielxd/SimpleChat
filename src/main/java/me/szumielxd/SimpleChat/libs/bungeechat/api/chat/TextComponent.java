...
/*	public static BaseComponent[] fromLegacyExtendedText(String message, ChatColor defaultColor) {
		ArrayList<TextComponent> components = new ArrayList<TextComponent>();
		StringBuilder builder = new StringBuilder();
		TextComponent component = new TextComponent();
		Matcher matcher = url.matcher(message);
		for (int i = 0; i < message.length(); ++i) {
			...
			if (c2 == '\u00a7') {
				...
				if (c2 == 'x' && i + 12 < message.length()) {
					StringBuilder hex = new StringBuilder("#");
					for (int j2 = 0; j2 < 6; ++j2) {
						hex.append(message.charAt(i + 2 + j2 * 2));
					}
					try {
						format = ChatColor.of(hex.toString());
					}
